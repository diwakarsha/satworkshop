...
import { satworkshop_backend } from "../../declarations/satworkshop_backend";
...
document.addEventListener(
	...
	async (e) => {
		...
		const question = await satworkshop_backend.getQuestion();
		document.getElementById("question").innerText = question;

		const voteCounts = await satworkshop_backend.getVotes();
...
pollForm.addEventListener(
	...
	async (e) => {
		...
		const updatedVoteCounts = await satworkshop_backend.vote(checkedValue);
...
resetButton.addEventListener(
	...
	async (e) => {
		...
		await satworkshop_backend.resetVotes();
		const voteCounts = await satworkshop_backend.getVotes();
		updateLocalVoteCounts(voteCounts);
